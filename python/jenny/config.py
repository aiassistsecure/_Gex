"""
Jenny Runtime Configuration
Pydantic-based settings loaded from jenny.config.json, env vars, and CLI args.
"""
...
class GeneConfig(BaseSettings):
    """Central configuration for the Jenny Python runtime."""
    ...
    # Paths
    data_dir: str = Field(default="", description="Jenny data directory")
    ...
    def load(cls, config_path: Optional[str] = None) -> "GeneConfig":
        ...
        # Try loading from jenny.config.json
        paths_to_try = [
            config_path,
            os.environ.get("JENNY_CONFIG_PATH"),
            "jenny.config.json",
            str(Path.home() / ".jenny" / "config.json"),
        ]
