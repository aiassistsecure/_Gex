"""
Gene FastAPI Server
The main HTTP + WebSocket server for the Gene Python runtime.
"""
...
from gene.config import GeneConfig, get_config, set_config
from gene.websocket import ConnectionManager
...
async def lifespan(app: FastAPI):
    ...
    config = get_config()
    logger.info(f"🧬 Gene Runtime v1.0.0 starting on {config.host}:{config.port}")
    logger.info(f"   Workspace: {config.workspace_dir}")
    logger.info(f"   Debug: {config.debug}")

    yield

    logger.info("🧬 Gene Runtime shutting down...")
...
def create_app(config: GeneConfig | None = None) -> FastAPI:
    ...
    app = FastAPI(
        title="Gene Runtime",
        description="Python intelligence layer for Gene desktop applications",
        version="1.0.0",
        lifespan=lifespan,
    )
    ...
    from gene.routes import agent_routes, project_routes, template_routes
...
# ─── Entry Point ────────────────────────────────────────────────────
def main():
    """Launch the Gene runtime server."""
    logging.basicConfig(
        level=logging.INFO,
    ...
    logger.info("🧬 Gene Runtime — The Python Intelligence Layer")
